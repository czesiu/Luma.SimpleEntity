...
namespace Luma.SimpleEntity
{
    ...
    internal static class CodeGenUtilities
    {
        ...
        private static bool _isVisualBasic;
        private static bool _useFullTypeNames;
        private static string _rootNamespace;
        private static Dictionary<Assembly, Dictionary<string, string>> _contractNamespaces;
        ...
        private static Dictionary<Tuple<CodeNamespace, Type>, CodeTypeReference> _codeTypeReferences;

        private static Dictionary<Type, string> _typeNamespaceTranslations;
        private static Dictionary<string, Dictionary<string, string>> _namespaceTypeReferences;
        ...
        internal static void Initialize(bool isVisualBasic, bool useFullTypeNames, string rootNamespace)
        {
            ...
            _contractNamespaces = new Dictionary<Assembly, Dictionary<string, string>>();
            _codeTypeReferences = new Dictionary<Tuple<CodeNamespace, Type>, CodeTypeReference>();
            _namespaceTypeReferences = new Dictionary<string, Dictionary<string, string>>();
            _typeNamespaceTranslations = new Dictionary<Type, string>();
        }
        ...
        internal static string GetContractNamespace(Type entityType)
        {
            ...
            if (!CodeGenUtilities._contractNamespaces.TryGetValue(entityType.Assembly, out contractNamespaces))
            {
                ...
                _contractNamespaces.Add(entityType.Assembly, contractNamespaces);
            }
        ...
        private static bool RegisterTypeName(string typeNamespace, string typeName, string containingNamespace)
        {
            ...
            if (_isVisualBasic || (containingNamespace == null))
            {
            ...
            // Register namespace
            if (!_namespaceTypeReferences.ContainsKey(containingNamespace))
            {
                _namespaceTypeReferences.Add(containingNamespace, new Dictionary<string, string>(StringComparer.Ordinal));
            }
            ...
            // Check for conflict
            isConflict =
                _namespaceTypeReferences[containingNamespace].TryGetValue(typeName, out existingTypeName) &&
                existingTypeName != fullTypeName;

            // Register namespace type reference
            if (!isConflict && existingTypeName == null)
            {
                _namespaceTypeReferences[containingNamespace].Add(typeName, fullTypeName);
            }
        ...
        private static string OptimizeAttributeName(Type type)
        {
            ...
            if (!_useFullTypeNames && typeof(Attribute).IsAssignableFrom(type))
            {
                string typeName = type.Name;
