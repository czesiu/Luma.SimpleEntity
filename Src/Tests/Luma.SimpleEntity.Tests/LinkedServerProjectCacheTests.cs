...
namespace Luma.SimpleEntity.Tests
{
    ...
    public class LinkedServerProjectCacheTests
    {
        ...
        public void LinkedServerProjectCache_ServerProjectLinks()
        {
            ConsoleLogger logger = new ConsoleLogger();
            using (ProjectFileReader projectFileReader = new ProjectFileReader(logger))
            {
                var projectFile = CreateMockProjectFile();
                var historyFile = CreateMockHistoryFile();

                try
                {
                    var cache = new LinkedServerProjectCache(projectFile, historyFile, logger, projectFileReader);
                    ...
                    var projectRefs = cache.ProjectReferences.ToList();
                    ...
                    var serverProjectLinks = cache.LinkedServerProjects.ToList();
                    Assert.AreEqual(2, serverProjectLinks.Count, "Expected this many project references to have RIA Links");
                    Assert.IsTrue(serverProjectLinks.Contains("w1"), "expected p1 to be in list of RIA links");
                    Assert.IsTrue(serverProjectLinks.Contains("w3"), "expected p3 to be in list of RIA links");
                }
                finally
                {
                    File.Delete(projectFile);
        ...
        public void LinkedServerProjectCache_LoadAndSaveFile()
        {
            ...
            using (var projectFileReader = new ProjectFileReader(logger))
            {
            ...
            using (var projectFileReader = new ProjectFileReader(logger))
            {
                ...
                string refProject1Contents = string.Format(@"
                    <Project ToolsVersion=""4.0"" DefaultTargets=""Build"" xmlns=""http://schemas.microsoft.com/developer/msbuild/2003"">
                      <PropertyGroup>
                        <LinkedSimpleEntityServerProject>{0}</LinkedSimpleEntityServerProject>
                      </PropertyGroup>
